...
my %final_scores = ();
open(F, "$raca_dir/SFs/block_combinedscores.flt.bid.txt");
while(<F>) {
    chomp;
    my ($bid1, $bid2, $score) = split(/\s+/);
	my ($rbid1, $rbid2) = (-1*$bid1, -1*$bid2);
	$final_scores{"$bid1:$bid2"} = $score;
	$final_scores{"$rbid2:$rbid1"} = $score;
}
close(F);

if($ef){
	open(F, "$raca_dir/fill/scf_block_combinedscores.flt.bid.txt");
	while(<F>) {
	    chomp;
	    my ($bid1, $bid2, $score) = split(/\s+/);
		my ($rbid1, $rbid2) = (-1*$bid1, -1*$bid2);
...
my %lr_intra_reads = ();
if ($type =~ /LR/) {
    read_link_data("$raca_dir/SFs/block_linkscores.LR.bid.txt", $lr_inter_f, $lr_intra_f, \%lr_stat, \%lr_link_scores, \%lr_inter_reads, \%lr_intra_reads);
}
...
while(<F>) {
	...
	for (my $i = 0; $i < $#ar; $i++) {
		...
		my $rchr1 = (exists $block_rchr{abs($bid1)})? $block_rchr{abs($bid1)} : "na";
		my $rchr2 = (exists $block_rchr{abs($bid2)})? $block_rchr{abs($bid2)} : "na";

		if ($rchr1 eq $rchr2 && $rchr1 ne "na") {	
        ...
        my $sr_label = "RN";
		if ($type =~ /SR/) {
		    my $sr_score = $sr_link_scores{"$bid1:$bid2"};
		    if (defined($sr_score)) {
        ...
        my $linkr_label = "RN";
		if ($type =~ /LnR/) {
		    my $linkr_score = $linkr_link_scores{"$bid1:$bid2"};
        ...
        my $lr_label = "RN";
		if ($type =~ /LR/) {
		    my $lr_score = $lr_link_scores{"$bid1:$bid2"};
		    if (defined($lr_score)) {
		...
		if (defined $final_scores{"$bid1:$bid2"}) {
        	my $fscore = $final_scores{"$bid1:$bid2"};
...
open(O, ">$raca_dir/adjinfo.txt");
print O "#chr\tbid1:bid2\tcons_score\t";
foreach my $spc (@spcs) {
	print O "$spc\t";
}
...
if ($type =~ /LnR/) { print O "10xR_label\t"; }
...
open(O, ">$raca_dir/adjinfo_sorted.txt");
print O "#chr\tbid1:bid2\tcons_score\t";
foreach my $spc (@spcs) {
	print O "$spc\t";
}
...
open(O, ">$raca_dir/adjinfo_nonref.txt");
print O "#chr\tbid1:bid2\tcons_score\t";
foreach my $spc (@spcs) {
	print O "$spc\t";
}
